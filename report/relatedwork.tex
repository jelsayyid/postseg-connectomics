...
The CAVE (Connectome Annotation Versioning Engine)~\cite{dorkenwald2025cave}
...
A distinct family of recent work operates on mesh or skeleton
representations of segmented neurons rather than on raw voxels. Joyce et
al.~\cite{joyce2023mesh} identify false splits by detecting jagged or
irregular geometry near neuronal tips in 3D meshes.
NEURD~\cite{celii2025neurd} transforms 3D neuron meshes into annotated
graph representations and applies heuristic graph rules for
merge-error correction. ConnectomeBench~\cite{brown2025connectomebench}
frames proofreading as a set of multimodal reasoning tasks over 3D mesh
renderings and evaluates large multimodal models on split-error
correction, segment-type identification, and merge-error detection.
Our pipeline sits in the same mesh/skeleton-based tradition: the
computational core operates on TEASAR skeleton nodes and fragment-level
summary statistics, with the voxel-level segmentation used only as
input (for fragment extraction) and output (for Neuroglancer rendering),
never as a decision-making substrate.
...
ground-truth merge pairs for training and evaluation. To our knowledge, this
...
full skeleton-node indexing and quantifies the impact of each on ground-truth coverage,
