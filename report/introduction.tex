...
Manual proofreading, which requires human experts to identify and repair segmentation
...
essential infrastructure for the field~\cite{dorkenwald2025cave}.

\section*{The Problem This Work Addresses}

This project targets split detection and correction proposal in
...
The domain of primary interest is myelinated axons in mouse white matter,
...
\begin{enumerate}
  \item A complete, modular post-segmentation pipeline implemented in
  ...
  \item The skeleton-node graph construction method, which indexes every
        ...
        a 47-percentage-point improvement in ground-truth coverage on XPRESS (from 23\%
        to 79.8\% in a single change).

  \item Distance-conditioned composite scoring, which switches the
  ...
  \item A three-outcome validation model (ACCEPT / REJECT / AMBIGUOUS)
  ...
  \item Quantitative evaluation on two benchmarks: CREMI (F1 = 0.952,
        Precision = 1.000) and XPRESS (Recall = 0.9958 on training, Recall = 0.9879
        on a fully held-out validation volume), with 28 documented experiments
